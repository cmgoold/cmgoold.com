...
def common_causes():
    ...
    print("Wrong: ", (
        (
            ...
            pm.stats.hdi(idata.posterior.beta, 0.95)
        ),
        (
    ...
    with pm.Model() as model:
      ...
      eta = pm.math.invlogit(alpha + beta0 * (immigration - 0.5) + beta1 * sex_z + beta2 * age_z)
      likelihood = pm.Binomial("crime", n=1, p=eta, observed=crime)
      idata = pm.sample(1000, tune=1000, random_seed=rng)

    print("Right: ", (
        (
            idata.posterior.beta0.mean(),
            pm.stats.hdi(idata.posterior.beta0, 0.95)
        ),
        (
